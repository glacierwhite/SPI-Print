...
We adopt an identity-guided conditional latent diffusion architecture for fingerprint super-resolution. Specifically, We use the pretrained AutoencoderKL from Stable Diffusion as the variational autoencoder (VAE). The VAE is then frozen during the training/inference of the latent diffusion model and encodes the HR fingerprint image into a compact latent representation. During super-resolution, the LR fingerprint reconstructed by SPI is processed by a lightweight trainable condition encoder to produce conditioning embeddings. A denoising network–which is the pretrained U-Net adopted from Stable Diffusion with its input blocks frozen while the middle and output blocks trainable–then performs iterative conditional denoising in the latent domain, starting from Gaussian noise and progressively refining the latent code under the guidance of the conditioning embeddings and diffusion timestep embeddings. The final latent representation is decoded by the frozen VAE decoder to obtain a HR fingerprint. 
